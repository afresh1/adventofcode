...
multi sub postfix:<+_&> ($a --> Int:D) { $a +& ( 2 ** 16 - 1 ) }

sub next-a (Int $current) { ( $current * 16807 ) % 2147483647 }
sub next-b (Int $current) { ( $current * 48271 ) % 2147483647 }
sub next-values( @current ) {
    return ( next-a(@current[0]), next-b(@current[1]) );
}

sub next-values-even( @current ) {
    my ($next-a, $next-b) = next-values(@current);
    $next-a = next-a($next-a) while $next-a % 4;
    $next-b = next-b($next-b) while $next-b % 8;
    return ( $next-a, $next-b );
}

sub make-generator(Int $start_a, Int $start_b) {
    return ( ( $start_a, $start_b ), &next-values ... * );
}
...
{
    my $gen = make-generator( 65, 8921 );

    # --Gen. A--  --Gen. B--
    is $gen[1], (   1092455,   430625591), "Matched 1";
    is $gen[2], (1181022009,  1233683848), "Matched 2";
    is $gen[3], ( 245556042,  1431495498), "Matched 3";
    is $gen[4], (1744312007,   137874439), "Matched 4";
    is $gen[5], (1352636452,   285222916), "Matched 5";

    # In binary, these pairs are (with generator A's value first in each pair):

    # .map(*+_&)

    is $gen[1].map( *.fmt("%032b") ), (
     "00000000000100001010101101100111",
     "00011001101010101101001100110111"
    ), "1 Matches in binary";

    is $gen[2].map( *.fmt("%032b") ), (
     "01000110011001001111011100111001",
     "01001001100010001000010110001000"
    ), "2 Matches in binary";

    is $gen[3].map( *.fmt("%032b") ), (
     "00001110101000101110001101001010",
     "01010101010100101110001101001010"
    ), "3 Matches in binary";

    is $gen[4].map( *.fmt("%032b") ), (
     "01100111111110000001011011000111",
     "00001000001101111100110000000111"
    ), "4 Matches in binary";

    is $gen[5].map( *.fmt("%032b") ), (
...
# these five pairs, the judge would have added only 1 to its total.
    ...
    is $gen[1...5].hyper
        .grep({ [==] $_.map(*+_&) }).elems,
        1, "Only a single pair matches the lower 16 bits";
}
...
# After 40 million pairs, what is the judge's final count?
    ...
    #my $gen = make-generator( 65, 8921 );
    my $start = ( 65, 8921 ); # test input, scores 588

{
    ...
    while True {
        $i++;
        last if $i >= 5_000_000;
        $v = next-values-even( $v );
        ( "$i: " ~ $v ).say if $i % 10_000 == 0;
        if [==] $v.map(*+_&) {
            $matched = $i;
            last;
        }
    ...
    is $matched, 1056, "First match after 1056 iterations";
}
...
if (0) {
    ...
    while True {
        $i++;
        last if $i >= 40_000_000;
        $v = next-values( $v );
        ( "$i: " ~ $v ).say if $i % 10_000 == 0;
        $matches++ if [==] $v.map(*+_&);
    }

    is $matches, 388, "So many matching pairs";
}
...
if (0) {
    ...
    is $matches, 1, "So many matching pairs";
}
...

{
    ...
    while True {
        ...
        last if $i >= 5_000_000;
        $v = next-values-even( $v );
        ( "$i: " ~ $v.map(*.fmt("%12d")) ).say if $i % 10_000 == 0;
        $matches++ if [==] $v.map(*+_&);
    }

    is $matches, 294, "So many matching pairs";
}
