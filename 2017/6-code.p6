...
#     2 4 1 2.
    ...
    reallocate(@bank);
    is @bank, < 2 4 1 2 >, "First reallocation";
...
#     result is: 3 1 2 3.
    ...
    reallocate(@bank);
    is @bank, < 3 1 2 3 >, "Second reallocation";
...
{
    my @list = "6-input".IO.lines.comb(/\d+/).map({.Int});
    my $count = 0;
    my %seen;
    while (not %seen{ @list.join(':') }++) {
        $count++;
        reallocate(@list);
    }
    say "Allocated duplicate for input in $count steps";
...
# in your puzzle input?
    ...
    repeat {
        $count++;
        reallocate(@list);
    } while ( $dup ne @list.join(':') ) ;
    say "Allocated duplicate a second time in $count steps";
}
